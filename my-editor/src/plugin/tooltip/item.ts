...
export const inputMap = (ctx: LoadPluginContext): InputMap => {
  ...
  return {
    ...
    [InputAction.ModifyImage]: {
      ...
      update: (view, $) => {
        const inputs = $.getElementsByClassName('input-img')

        const node = findChildNode(view.state.selection, nodes.image)
        const img = document.querySelector('.react-image')
        if (!node || !img) return

        if (inputs) {
          ;(inputs[1] as HTMLInputElement).value = node.node.attrs.src
          ;(inputs[0] as HTMLInputElement).value = img.clientWidth.toString()
        }
      },
    },
  }
...
export const buttonMap = (ctx: LoadPluginContext): ButtonMap => {
  ...
  return {
    ...
    [ButtonAction.ToggleLink]: createToggleIcon('link', marks.link, marks.code_inline, {
      href: '',
    }),
  }
}
